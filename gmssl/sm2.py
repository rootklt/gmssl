...
class CryptSM2(GenSM2KEY):
    ...
    def _kg(self, k: int, Point: bytes) -> bytes:
        ...
        '''
        Point += b'1'
        mask_str = b'8' + b'0'*(self.para_len - 1)
    ...
    def _double_point(self, Point: bytes) -> bytes:  # 倍点
        l = len(Point)
    ...
    def _add_point(self, P1: bytes, P2: bytes) -> bytes:
        '''
    ...
    def _convert_jacb_to_nor(self, Point: bytes) -> bytes:  # Jacobian加重射影坐标转换成仿射坐标
        len_2 = 2 * self.para_len
    ...
    def verify(self, Sign: bytes, data: bytes):
        ...
        P1 = self._kg(s, f"{self.ecc_table['g']:x}".encode())
    ...
    def sign(self, data: bytes, K: bytes) -> bytes:
        ...
        P1 = self._kg(k, f"{self.ecc_table['g']:x}".encode())
    ...
    def encrypt(self, plaintext: bytes) -> bytes:
        ...
        C1 = self._kg(int(k, 16), f"{self.ecc_table['g']:x}".encode())
        ...
        if int(t, 16) == 0:
            return b''
        form = f'{{:0{ml}x}}'
        C2 = form.format(int(plaintext, 16) ^ int(t, 16)).encode()
        C3 = sm3.sm3_hash(f'{x2}{plaintext}{y2}'.encode())

        return unhexlify(C1+C3+C2) if self.mode else unhexlify(C1+C2+C3)
    ...
    def _sm3_z(self, data: bytes):
        ...
        z = unhexlify(b'0080'+b'31323334353637383132333435363738' +
                      f"{self.ecc_table['a']:x}{self.ecc_table['b']:x}{self.ecc_table['g']:x}".encode() + self.public_key)

        Za = sm3.sm3_hash(z)
        return sm3.sm3_hash(Za + data)
